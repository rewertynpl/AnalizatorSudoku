...
enum class Strategy {
    ...
    XWing, YWing, XYZWing, Swordfish, Jellyfish,
    Skyscraper, TwoStringKite, SimpleColoring, ThreeDMedusa, FinnedXWingSashimi, EmptyRectangle,
    UniqueRectangle, AvoidableRectangle, BUGPlus1,
    RemotePairs, WWing, GroupedXCycle, XChain, XYChain, GroupedAIC, AIC, ALSXZ, SueDeCoq, Exocet,
...
static std::string strategyImplementationStatus(Strategy s) {
    switch (s) {
        ...
        case Strategy::UniqueRectangle:
            return "ZAIMPLEMENTOWANE (Type 1-6, wariant konserwatywny)";
...
private:
    ...
    bool hasSolutionWithCandidate(int cell, int digit) const;
    bool hasSolutionWithAssignments(const std::vector<std::pair<int, int>>& assignments) const;
...
bool SudokuAnalyzer::hasSolutionWithCandidate(int cell, int digit) const {
    if (cell < 0 || cell >= NN_ || digit < 1 || digit > N_) return false;
    if (grid_[cell] != 0 && grid_[cell] != digit) return false;
    if ((cand_[cell] & bit(digit)) == 0U) return false;

    std::vector<int> g = grid_;
    g[cell] = digit;
    BacktrackingCounter bt(BR_, BC_, N_, g);
    return bt.countSolutions(1) > 0;
}

bool SudokuAnalyzer::hasSolutionWithAssignments(const std::vector<std::pair<int, int>>& assignments) const {
    std::vector<int> g = grid_;
    for (const auto& asg : assignments) {
        const int cell = asg.first;
        const int digit = asg.second;
        if (cell < 0 || cell >= NN_ || digit < 1 || digit > N_) return false;
        if (grid_[cell] != 0 && grid_[cell] != digit) return false;
        if (grid_[cell] == 0 && (cand_[cell] & bit(digit)) == 0U) return false;
        g[cell] = digit;
    }
    BacktrackingCounter bt(BR_, BC_, N_, g);
    return bt.countSolutions(1) > 0;
                ...
                for (int c2 = c1 + 1; c2 < N_; ++c2) {
                    ...
                    for (std::size_t i = 0; i < commonDigits.size(); ++i) {
                        for (std::size_t j = i + 1; j < commonDigits.size(); ++j) {
                            ...
                            // Type 3/5/6 (konserwatywnie): eliminacje z roofow tylko gdy kandydat nie ma zadnego rozwiazania.
                            int localByCheck = 0;
                            for (int r : roofs) {
                                const unsigned int extras = cand_[r] & ~pairMask;
                                unsigned int m = extras;
                                while (m) {
                                    const unsigned int one = m & (~m + 1U);
                                    const int z = firstDigit(one);
                                    if (!hasSolutionWithCandidate(r, z)) {
                                        bool changed = false;
                                        if (!removeCandidate(r, z, changed)) return false;
                                        if (changed) ++localByCheck;
                                    }
                                    m &= (m - 1U);
                                }
                            }
                            if (localByCheck > 0) { n = localByCheck; return true; }
            ...
            for (std::size_t d1 = 0; d1 < cd.size(); ++d1) {
                for (std::size_t d2 = d1 + 1; d2 < cd.size(); ++d2) {
                    ...
                    for (const auto& mode : mapModes) {
                        ...
                        std::map<std::string, bool> satCache;
                        auto sat = [&](std::vector<std::pair<int, int>> asg) -> bool {
                            ...
                            const auto it = satCache.find(k);
                            if (it != satCache.end()) return it->second;
                            const bool ok = hasSolutionWithAssignments(asg);
                            satCache[k] = ok;
                            return ok;
                        };
                        ...
                        for (const Perm& p : perms) {
                            ...
                            for (int t : branch1) {
                                ...
                                if (sat(asg)) { support1 = true; break; }
                            }
                            ...
                            for (int t : branch2) {
                                ...
                                if (sat(asg)) { support2 = true; break; }
                            }
                            ...
                            for (int t : branch) {
                                ...
                                for (int d : dlist) {
                                    ...
                                    for (const Perm& p : feasiblePerms) {
                                        ...
                                        if (sat(asg)) { supported = true; break; }
                                    }
                                    if (!supported) {
                                        // Bezpiecznik: nie usuwaj, jesli kandydat ma jakiekolwiek globalne wsparcie.
                                        if (sat({{t, d}})) continue;
                                        bool changed = false;
        ...
        for (int c : probeCells) {
            ...
            for (int d : dlist) {
                ...
                for (const auto& br : feasible) {
                    bool supported = false;
                    if (br.first == c) {
                        supported = (br.second == d);
                    } else {
                        supported = sat({br, {c, d}});
                    }
                    if (supported) {
    ...
    auto evalBranches = [&](const std::vector<std::pair<int, int>>& branches) -> bool {
        ...
        for (int c = 0; c < NN_; ++c) {
            ...
            for (int d : digitsFromMask(cand_[c])) {
                ...
                for (const auto& br : feasible) {
                    if (br.first == c) {
                        if (br.second == d) { supported = true; break; }
                    } else if (sat({br, {c, d}})) {
                        supported = true;
                        break;
                    }
...
static void writeFolderReport(const fs::path& outDir, const std::string& folderKey, const FolderStats& stats) {
    ...
    const std::vector<Strategy> order = {
        ...
        Strategy::XWing, Strategy::YWing, Strategy::XYZWing, Strategy::Swordfish, Strategy::Jellyfish,
        Strategy::Skyscraper, Strategy::TwoStringKite, Strategy::SimpleColoring, Strategy::ThreeDMedusa,
        Strategy::FinnedXWingSashimi, Strategy::EmptyRectangle,
        Strategy::UniqueRectangle, Strategy::AvoidableRectangle, Strategy::BUGPlus1,
        Strategy::RemotePairs, Strategy::WWing, Strategy::GroupedXCycle, Strategy::XChain, Strategy::XYChain,
        Strategy::GroupedAIC, Strategy::AIC, Strategy::ALSXZ, Strategy::SueDeCoq, Strategy::Exocet,
...
static void writeFolderCsv(const fs::path& outDir, const std::map<std::string, FolderStats>& allStats) {
    ...
    out << "folder,non_empty_lines,analyzed_puzzles,invalid_lines,solved_logically,requires_guessing,"
        ...
        << "x_wing,y_wing,xyz_wing,swordfish,jellyfish,skyscraper,two_string_kite,simple_coloring,three_d_medusa,"
        << "finned_x_wing_sashimi,empty_rectangle,unique_rectangle,avoidable_rectangle,bug_plus_1,"
        << "remote_pairs,w_wing,grouped_x_cycle,x_chain,xy_chain,grouped_aic,aic,als_xz,sue_de_coq,exocet,forcing_chains,backtracking\n";
