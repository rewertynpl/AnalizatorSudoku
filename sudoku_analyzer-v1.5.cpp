...
struct SudokuBoard {
    ...
    std::vector<int> source_solution;
    bool valid = false;
    std::string error;
};
...
static int bits(uint64_t m) { int c = 0; while (m) { m &= (m - 1ULL); ++c; } return c; }
static int firstDigit(uint64_t m) {
    int d = 1;
    while (m) { if (m & 1ULL) return d; m >>= 1ULL; ++d; }
    return 0;
}
static std::vector<int> digitsFromMask(uint64_t m) {
    ...
    int d = 1;
    while (m) {
        if (m & 1ULL) out.push_back(d);
        m >>= 1ULL;
        ++d;
    }
    return out;
}
static int maskMinDigit(uint64_t m) {
    int d = 1;
    while (m) {
        if (m & 1ULL) return d;
        m >>= 1ULL;
...
static std::string strategyImplementationStatus(Strategy s) {
    switch (s) {
        ...
        case Strategy::WXYZWing:
            return "ZAIMPLEMENTOWANE (z walidacja eliminacji)";
...
private:
    ...
    bool strict_logic_guard_ = true;
    bool in_logical_phase_ = false;
    bool in_initialization_ = false;
    bool source_solution_available_ = false;
    std::vector<int> source_solution_;
    Strategy current_strategy_ = Strategy::NakedSingle;
    std::array<long long, 10> strategy_bug_rejections_{};
    std::vector<std::string> strategy_bug_logs_;
    std::size_t strategy_bug_log_limit_ = 200;
    ...
    void recordStrategyBug(const char* action, int cell, int attemptedDigit, int expectedDigit);
    bool isGuardedAdvancedStrategy(Strategy s) const;
    bool isEliminationSafeBySearch(int cell, int digit) const;
    bool isAssignmentForcedBySearch(int cell, int digit) const;
...
static std::string formatSudokuGrid(const std::vector<int>& values, int sideSize, int blockRows, int blockCols);
static void writePerPuzzleSolveOutput(const fs::path& outDir, const PuzzleResult& result);
...
SudokuAnalyzer::SudokuAnalyzer(const SudokuBoard& b) : b_(b) {
    ...
    BC_ = b_.block_cols;
    NN_ = N_ * N_;
    all_ = (N_ >= 64) ? 0ULL : ((1ULL << N_) - 1ULL);
    source_solution_available_ = (static_cast<int>(b_.source_solution.size()) == NN_);
    if (source_solution_available_) {
        source_solution_ = b_.source_solution;
        for (int v : source_solution_) {
            if (v < 1 || v > N_) {
                source_solution_available_ = false;
                source_solution_.clear();
                break;
            }
        }
    }
...
void SudokuAnalyzer::recordStrategyBug(const char* action, int cell, int attemptedDigit, int expectedDigit) {
    int lvl = strategyRank(current_strategy_);
    if (lvl < 1) lvl = 1;
    if (lvl > 9) lvl = 9;
    ++strategy_bug_rejections_[lvl];

    if (strategy_bug_logs_.size() >= strategy_bug_log_limit_) {
        return;
    }
    ...
    ss << "BUG[" << action << "]"
       << " lvl=" << lvl
       << " strategia=" << strategyName(current_strategy_)
       << " komorka=" << cellName(cell)
       << " probowano=" << attemptedDigit
       << " oczekiwane=" << expectedDigit;
    strategy_bug_logs_.push_back(ss.str());
}

bool SudokuAnalyzer::isGuardedAdvancedStrategy(Strategy s) const {
    switch (s) {
        case Strategy::FrankenMutantFish:
        case Strategy::WXYZWing:
        case Strategy::GroupedXCycle:
...
bool SudokuAnalyzer::isEliminationSafeBySearch(int cell, int digit) const {
    if (cell < 0 || cell >= NN_ || digit < 1 || digit > N_) return false;
    if (grid_[cell] != 0) return grid_[cell] != digit;
    if ((cand_[cell] & bit(digit)) == 0U) return true;

    std::vector<int> g = grid_;
    g[cell] = digit;
    BacktrackingCounter bt(BR_, BC_, N_, std::move(g));
    const int solutions = bt.countSolutions(1);
    return (solutions == 0);
}

bool SudokuAnalyzer::isAssignmentForcedBySearch(int cell, int digit) const {
    if (cell < 0 || cell >= NN_ || digit < 1 || digit > N_) return false;
    if (grid_[cell] != 0) return grid_[cell] == digit;
    if ((cand_[cell] & bit(digit)) == 0U) return false;

    std::vector<int> g = grid_;
    g[cell] = digit;
    {
        BacktrackingCounter bt(BR_, BC_, N_, g);
        if (bt.countSolutions(1) == 0) {
            return false;
        }
    }

    for (int alt = 1; alt <= N_; ++alt) {
        if (alt == digit) continue;
        if ((cand_[cell] & bit(alt)) == 0U) continue;
        std::vector<int> ga = grid_;
        ga[cell] = alt;
        BacktrackingCounter btAlt(BR_, BC_, N_, std::move(ga));
        if (btAlt.countSolutions(1) > 0) {
            return false;
        }
    }
    return true;
...
bool SudokuAnalyzer::removeCandidate(int i, int d, bool& changed) {
    if (strict_logic_guard_ && in_logical_phase_) {
        if (!isEliminationSafeBySearch(i, d)) {
            std::ostringstream ss;
            ss << "Guard: odrzucono eliminacje " << d << " z " << cellName(i)
               << " dla strategii " << strategyName(current_strategy_);
            pushDebugLog(ss.str());
            return true;
        }
    }

    if (source_solution_available_ && i >= 0 && i < NN_) {
        const int expected = source_solution_[i];
        if (expected == d) {
            recordStrategyBug("usuniecie_kandydata_rozwiazania", i, d, expected);
            return true;
        }
    }
...
bool SudokuAnalyzer::assignValue(int i, int d) {
    if (strict_logic_guard_ && in_logical_phase_ && !in_initialization_) {
        if (!isAssignmentForcedBySearch(i, d)) {
            if (source_solution_available_ && i >= 0 && i < NN_) {
                const int expected = source_solution_[i];
                recordStrategyBug("niewymuszone_przypisanie", i, d, expected);
            }
            std::ostringstream ss;
            ss << "Guard: odrzucono przypisanie " << d << " do " << cellName(i)
               << " dla strategii " << strategyName(current_strategy_);
            pushDebugLog(ss.str());
            return true;
        }
    ...
    if (source_solution_available_ && i >= 0 && i < NN_) {
        const int expected = source_solution_[i];
        if (expected >= 1 && expected <= N_ && expected != d) {
            recordStrategyBug("bledne_przypisanie", i, d, expected);
            return true;
        }
    }
...
bool SudokuAnalyzer::applyFrankenMutantFish(int size, int& n) {
    ...
    auto collectEligible = [&](const std::vector<int>& pool, int d) {
        std::vector<int> out;
        for (int h : pool) {
            int cnt = 0;
            for (int idx : houses_[h]) {
                if (grid_[idx] == 0 && (cand_[idx] & bit(d))) ++cnt;
            }
            if (cnt >= 2 && cnt <= 6) out.push_back(h);
        }
        return out;
    };

    auto runMode = [&](const std::vector<int>& basePool,
                       const std::vector<int>& coverPool,
                       const char* modeName) -> bool {
        for (int d = 1; d <= N_; ++d) {
            const std::vector<int> baseEligible = collectEligible(basePool, d);
            const std::vector<int> coverEligible = collectEligible(coverPool, d);
            if (static_cast<int>(baseEligible.size()) < size) continue;
            if (static_cast<int>(coverEligible.size()) < size) continue;
            ...
            forEachCombo(baseEligible, size, [&](const std::vector<int>& baseSets) {
                ...
                std::vector<int> baseCells;
                for (int h : baseSets) {
                    for (int idx : houses_[h]) {
                        if (grid_[idx] != 0 || (cand_[idx] & bit(d)) == 0U) continue;
                        if (!inBase[idx]) {
                            inBase[idx] = 1;
                            baseCells.push_back(idx);
                        }
                    }
                }
                if (baseCells.size() < static_cast<std::size_t>(size + 1)) return;

                forEachCombo(coverEligible, size, [&](const std::vector<int>& coverSets) {
                    if (found || contradiction_) return;

                    std::vector<char> inCover(NN_, 0);
                    bool everyCoverIntersectsBase = true;
                    for (int h : coverSets) {
                        bool touchesBase = false;
                        for (int idx : houses_[h]) {
                            if (grid_[idx] != 0 || (cand_[idx] & bit(d)) == 0U) continue;
                            inCover[idx] = 1;
                            if (inBase[idx]) touchesBase = true;
                        }
                        if (!touchesBase) {
                            everyCoverIntersectsBase = false;
                            break;
                        }
                    }
                    if (!everyCoverIntersectsBase) return;

                    for (int idx : baseCells) {
                    ...
                    if (local > 0) {
                        std::ostringstream ss;
                        ss << "FrankenMutantFish(" << size << "," << modeName << "): remove "
                           << d << " from " << local << " cell(s)";
                        pushDebugLog(ss.str());
                        n = local;
                        found = true;
    ...
    std::vector<int> rows, cols, boxes;
    for (int i = 0; i < N_; ++i) {
        rows.push_back(i);
        cols.push_back(N_ + i);
        boxes.push_back(2 * N_ + i);
    }

    std::vector<int> rowCover = cols;
    rowCover.insert(rowCover.end(), boxes.begin(), boxes.end());
    std::vector<int> colCover = rows;
    colCover.insert(colCover.end(), boxes.begin(), boxes.end());
    std::vector<int> boxCover = rows;
    boxCover.insert(boxCover.end(), cols.begin(), cols.end());

    if (runMode(rows, rowCover, "row")) return true;
    if (runMode(cols, colCover, "col")) return true;
    return runMode(boxes, boxCover, "box");
}
...
bool SudokuAnalyzer::applyWXYZWing(int& n) {
    n = 0;
    for (int pivot = 0; pivot < NN_; ++pivot) {
        if (grid_[pivot] != 0) continue;
        const int pivotBits = bits(cand_[pivot]);
        if (pivotBits < 3 || pivotBits > 4) continue;

        std::vector<int> wings;
        for (int p : peers_[pivot]) {
            if (grid_[p] != 0) continue;
            if (bits(cand_[p]) != 2) continue;
            wings.push_back(p);
        }
        ...
        for (std::size_t i = 0; i < wings.size(); ++i) {
            ...
            for (std::size_t j = i + 1; j < wings.size(); ++j) {
                ...
                for (std::size_t k = j + 1; k < wings.size(); ++k) {
                    ...
                    const std::array<int, 4> pattern = {pivot, w1, w2, w3};

                    uint64_t unionMask = 0U;
                    for (int c : pattern) unionMask |= cand_[c];
                    if (bits(unionMask) != 4) continue;

                    bool coverOk = true;
                    for (int d : digitsFromMask(unionMask)) {
                        int cnt = 0;
                        for (int c : pattern) {
                            if (cand_[c] & bit(d)) ++cnt;
                        }
                        if (cnt < 2) {
                            coverOk = false;
                            break;
                    ...
                    for (int z : digitsFromMask(unionMask)) {
                        std::vector<int> zCells;
                        for (int c : pattern) {
                            if (cand_[c] & bit(z)) zCells.push_back(c);
                        }
                        if (zCells.size() < 2U) continue;

                        bool restricted = true;
                        for (std::size_t a = 0; a < zCells.size() && restricted; ++a) {
                            for (std::size_t b = a + 1; b < zCells.size(); ++b) {
                                if (!isPeerCell(zCells[a], zCells[b])) {
                                    restricted = false;
                                    break;
                                }
                            }
                        ...
                        if (local > 0) {
                            std::ostringstream ss;
                            ss << "WXYZWing(new): pivot " << cellName(pivot) << " remove " << z
                               << " from " << local << " cell(s)";
                            pushDebugLog(ss.str());
                            n = local;
                            return true;
    ...
    for (int d = 1; d <= N_; ++d) {
        ...
        for (const auto& rp : rowPairs) {
            ...
            for (const auto& cp : colPairs) {
                ...
                if (box(row(a), col(a)) != box(row(b), col(b))) continue;

                const int target = otherRow * N_ + otherCol;
                if (target == a || target == b) continue;
                if (grid_[target] != 0 || (cand_[target] & bit(d)) == 0U) continue;
                if (!isPeerCell(target, a) || !isPeerCell(target, b)) continue;

                bool changed = false;
                if (!removeCandidate(target, d, changed)) return false;
                if (changed) {
                    n = 1;
                    std::ostringstream ss;
                    ss << "TwoStringKite: remove " << d << " from " << cellName(target);
                    pushDebugLog(ss.str());
                    return true;
                }
            }
...
bool SudokuAnalyzer::applyXYChain(int& n) {
    ...
    auto otherDigitInCell = [&](int cell, int d) -> int {
        const uint64_t rest = cand_[cell] & ~bit(d);
        ...
        for (int z : sd) {
            ...
            std::function<bool(int, int, int)> dfs = [&](int curIdx, int sharedDigit, int len) -> bool {
                ...
                for (std::size_t ni = 0; ni < cells.size(); ++ni) {
                    ...
                    const int nextShared = otherDigitInCell(nxtCell, sharedDigit);
                    if (nextShared == 0) continue;
                    const int newLen = len + 1;

                    const bool closesChain =
                        (newLen >= 3) &&
                        (nextShared == z) &&
                        isPeerCell(sCell, nxtCell);
                    ...
                    if (closesChain) {
                        ...
                        if (local > 0) {
                            std::ostringstream ss;
                            ss << "XYChain(new): endpoint " << cellName(sCell)
                               << " - " << cellName(nxtCell) << ", remove " << z
                               << " from " << local << " cell(s)";
                            pushDebugLog(ss.str());
                            n = local;
                            return true;
...
bool SudokuAnalyzer::applyGroupedXCycle(int& n) {
    ...
    auto groupsMutuallyPeer = [&](const GroupNode& a, const GroupNode& b) -> bool {
        bool overlap = false;
        for (int ca : a.cells) {
            for (int cb : b.cells) {
                if (ca == cb) {
        ...
        for (int ca : a.cells) {
            for (int cb : b.cells) {
                if (!isPeerCell(ca, cb)) return false;
            }
        }
        return true;
    };

    auto seesWholeGroup = [&](int cell, const GroupNode& g) -> bool {
        for (int c : g.cells) {
            if (!isPeerCell(cell, c)) return false;
        }
        return true;
    };

    auto groupContainsCell = [&](const GroupNode& g, int cell) {
        for (int c : g.cells) if (c == cell) return true;
        return false;
    };

    for (int d = 1; d <= N_; ++d) {
        std::vector<GroupNode> nodes;
        std::array<std::array<int, 9>, 9> rowGroupId{};
        std::array<std::array<int, 9>, 9> colGroupId{};
        for (int r = 0; r < 9; ++r) for (int b = 0; b < 9; ++b) rowGroupId[r][b] = -1;
        for (int c = 0; c < 9; ++c) for (int b = 0; b < 9; ++b) colGroupId[c][b] = -1;
        ...
        for (int r = 0; r < 9; ++r) {
            for (int b = 0; b < 9; ++b) {
                ...
                if (!g.empty()) {
                    rowGroupId[r][b] = static_cast<int>(nodes.size());
        ...
        for (int c = 0; c < 9; ++c) {
            for (int b = 0; b < 9; ++b) {
                std::vector<int> g;
                for (int idx : houses_[9 + c]) {
                    if (box(row(idx), col(idx)) != b) continue;
                    if (grid_[idx] != 0 || (cand_[idx] & bit(d)) == 0U) continue;
                    g.push_back(idx);
                }
                if (!g.empty()) {
                    colGroupId[c][b] = static_cast<int>(nodes.size());
                    nodes.push_back({std::move(g)});
                }
            }
        }

        if (nodes.size() < 2U) continue;

        std::vector<std::vector<int>> edges(nodes.size());
        auto addEdge = [&](int a, int b) {
            if (a < 0 || b < 0 || a == b) return;
            if (std::find(edges[a].begin(), edges[a].end(), b) == edges[a].end()) edges[a].push_back(b);
            if (std::find(edges[b].begin(), edges[b].end(), a) == edges[b].end()) edges[b].push_back(a);
        };

        for (int r = 0; r < 9; ++r) {
            std::vector<int> ids;
            for (int b = 0; b < 9; ++b) if (rowGroupId[r][b] >= 0) ids.push_back(rowGroupId[r][b]);
            if (ids.size() == 2U) addEdge(ids[0], ids[1]);
        }
        for (int c = 0; c < 9; ++c) {
            std::vector<int> ids;
            for (int b = 0; b < 9; ++b) if (colGroupId[c][b] >= 0) ids.push_back(colGroupId[c][b]);
            if (ids.size() == 2U) addEdge(ids[0], ids[1]);
        }
        for (int b = 0; b < 9; ++b) {
            std::vector<int> rowIds;
            std::vector<int> colIds;
            for (int r = 0; r < 9; ++r) if (rowGroupId[r][b] >= 0) rowIds.push_back(rowGroupId[r][b]);
            for (int c = 0; c < 9; ++c) if (colGroupId[c][b] >= 0) colIds.push_back(colGroupId[c][b]);
            if (rowIds.size() == 2U) addEdge(rowIds[0], rowIds[1]);
            if (colIds.size() == 2U) addEdge(colIds[0], colIds[1]);
        }
        ...
        int compCount = 0;
        for (int s = 0; s < static_cast<int>(nodes.size()); ++s) {
            if (color[s] != -1 || edges[s].empty()) continue;
            std::vector<int> q = {s};
            color[s] = 0;
            comp[s] = compCount;
            for (std::size_t qi = 0; qi < q.size(); ++qi) {
        ...
        std::vector<std::array<bool, 2>> bad(compCount, {false, false});
        for (int a = 0; a < static_cast<int>(nodes.size()); ++a) {
            ...
            for (int b = a + 1; b < static_cast<int>(nodes.size()); ++b) {
                ...
                if (groupsMutuallyPeer(nodes[a], nodes[b])) {
                    bad[comp[a]][color[a]] = true;
                }
            }
        }

        int local = 0;
        std::vector<char> touched(NN_, 0);
        for (int id = 0; id < static_cast<int>(nodes.size()); ++id) {
            if (comp[id] < 0) continue;
            if (!bad[comp[id]][color[id]]) continue;
            for (int cell : nodes[id].cells) {
                if (touched[cell]) continue;
                touched[cell] = 1;
                bool changed = false;
                if (!removeCandidate(cell, d, changed)) return false;
                if (changed) ++local;
            }
        }
        if (local > 0) {
            std::ostringstream ss;
            ss << "GroupedXCycle(new): color contradiction remove " << d
               << " from " << local << " cell(s)";
            pushDebugLog(ss.str());
            n = local;
            return true;
        ...
        std::vector<std::vector<int>> byComp(compCount);
        for (int id = 0; id < static_cast<int>(nodes.size()); ++id) {
            if (comp[id] >= 0) byComp[comp[id]].push_back(id);
        }

        for (int cell = 0; cell < NN_; ++cell) {
            if (grid_[cell] != 0 || (cand_[cell] & bit(d)) == 0U) continue;

            for (int cid = 0; cid < compCount; ++cid) {
                bool sees0 = false;
                bool sees1 = false;
                for (int id : byComp[cid]) {
                    if (groupContainsCell(nodes[id], cell)) continue;
                    if (!seesWholeGroup(cell, nodes[id])) continue;
                    if (color[id] == 0) sees0 = true;
                    else sees1 = true;
                ...
                if (changed) {
                    std::ostringstream ss;
                    ss << "GroupedXCycle(new): trap remove " << d
                       << " from " << cellName(cell);
                    pushDebugLog(ss.str());
                    n = 1;
                    return true;
...
void SudokuAnalyzer::logicalSolve(int max_rank) {
    ...
    const bool enableExperimentalWXYZWing = true;
    const bool enableExperimentalGroupedXCycle = true;
    const bool enableExperimentalXYChain = true;

    bool progress = true;
    while (!contradiction_ && !solved() && progress) {
        progress = false;
        int n = 0;
        if (tryStep(Strategy::NakedSingle, [&](int& x) { return applyNakedSingles(x); }, n)) { use(Strategy::NakedSingle, n); progress = true; continue; }
        if (tryStep(Strategy::HiddenSingle, [&](int& x) { return applyHiddenSingles(x); }, n)) { use(Strategy::HiddenSingle, n); progress = true; continue; }
        if (tryStep(Strategy::NakedPair, [&](int& x) { return applyNakedSubset(2, x); }, n)) { use(Strategy::NakedPair, n); progress = true; continue; }
        if (tryStep(Strategy::HiddenPair, [&](int& x) { return applyHiddenSubset(2, x); }, n)) { use(Strategy::HiddenPair, n); progress = true; continue; }
        if (tryStep(Strategy::PointingPairsTriples, [&](int& x) { return applyPointingPairsTriples(x); }, n)) { use(Strategy::PointingPairsTriples, n); progress = true; continue; }
        if (tryStep(Strategy::BoxLineReduction, [&](int& x) { return applyBoxLineReduction(x); }, n)) { use(Strategy::BoxLineReduction, n); progress = true; continue; }
        if (tryStep(Strategy::NakedTriple, [&](int& x) { return applyNakedSubset(3, x); }, n)) { use(Strategy::NakedTriple, n); progress = true; continue; }
        if (tryStep(Strategy::HiddenTriple, [&](int& x) { return applyHiddenSubset(3, x); }, n)) { use(Strategy::HiddenTriple, n); progress = true; continue; }
        if (tryStep(Strategy::NakedQuad, [&](int& x) { return applyNakedSubset(4, x); }, n)) { use(Strategy::NakedQuad, n); progress = true; continue; }
        if (tryStep(Strategy::HiddenQuad, [&](int& x) { return applyHiddenSubset(4, x); }, n)) { use(Strategy::HiddenQuad, n); progress = true; continue; }
        if (tryStep(Strategy::XWing, [&](int& x) { return applyFish(2, x); }, n)) { use(Strategy::XWing, n); progress = true; continue; }
        if (tryStep(Strategy::YWing, [&](int& x) { return applyYWing(x); }, n)) { use(Strategy::YWing, n); progress = true; continue; }
        if (tryStep(Strategy::XYZWing, [&](int& x) { return applyXYZWing(x); }, n)) { use(Strategy::XYZWing, n); progress = true; continue; }
        if (enableExperimentalWXYZWing &&
            tryStep(Strategy::WXYZWing, [&](int& x) { return applyWXYZWing(x); }, n)) { use(Strategy::WXYZWing, n); progress = true; continue; }
        if (tryStep(Strategy::Swordfish, [&](int& x) { return applyFish(3, x); }, n)) { use(Strategy::Swordfish, n); progress = true; continue; }
        if (tryStep(Strategy::FinnedSwordfish, [&](int& x) { return applyFinnedFish(3, x); }, n)) { use(Strategy::FinnedSwordfish, n); progress = true; continue; }
        if (tryStep(Strategy::FrankenMutantFish, [&](int& x) { return applyFrankenMutantFish(2, x); }, n)) { use(Strategy::FrankenMutantFish, n); progress = true; continue; }
        if (tryStep(Strategy::KrakenFish, [&](int& x) { return applyKrakenFish(x); }, n)) { use(Strategy::KrakenFish, n); progress = true; continue; }
        if (tryStep(Strategy::Skyscraper, [&](int& x) { return applySkyscraper(x); }, n)) { use(Strategy::Skyscraper, n); progress = true; continue; }
        if (tryStep(Strategy::TwoStringKite, [&](int& x) { return applyTwoStringKite(x); }, n)) { use(Strategy::TwoStringKite, n); progress = true; continue; }
        if (tryStep(Strategy::SimpleColoring, [&](int& x) { return applySimpleColoring(x); }, n)) { use(Strategy::SimpleColoring, n); progress = true; continue; }
        if (tryStep(Strategy::ThreeDMedusa, [&](int& x) { return applyThreeDMedusa(x); }, n)) { use(Strategy::ThreeDMedusa, n); progress = true; continue; }
        if (tryStep(Strategy::Jellyfish, [&](int& x) { return applyFish(4, x); }, n)) { use(Strategy::Jellyfish, n); progress = true; continue; }
        if (tryStep(Strategy::FinnedXWingSashimi, [&](int& x) { return applyFinnedXWingSashimi(x); }, n)) { use(Strategy::FinnedXWingSashimi, n); progress = true; continue; }
        if (tryStep(Strategy::FinnedJellyfish, [&](int& x) { return applyFinnedFish(4, x); }, n)) { use(Strategy::FinnedJellyfish, n); progress = true; continue; }
        if (tryStep(Strategy::FrankenMutantFish, [&](int& x) { return applyFrankenMutantFish(3, x); }, n)) { use(Strategy::FrankenMutantFish, n); progress = true; continue; }
        if (tryStep(Strategy::EmptyRectangle, [&](int& x) { return applyEmptyRectangle(x); }, n)) { use(Strategy::EmptyRectangle, n); progress = true; continue; }
        if (tryStep(Strategy::UniqueRectangle, [&](int& x) { return applyUniqueRectangleType1(x); }, n)) { use(Strategy::UniqueRectangle, n); progress = true; continue; }
        if (tryStep(Strategy::UniqueRectangle, [&](int& x) { return applyUniqueRectangleType2to6(x); }, n)) { use(Strategy::UniqueRectangle, n); progress = true; continue; }
        if (tryStep(Strategy::UniqueLoop, [&](int& x) { return applyUniqueLoop(x); }, n)) { use(Strategy::UniqueLoop, n); progress = true; continue; }
        if (tryStep(Strategy::BivalueOddagon, [&](int& x) { return applyBivalueOddagon(x); }, n)) { use(Strategy::BivalueOddagon, n); progress = true; continue; }
        if (tryStep(Strategy::AvoidableRectangle, [&](int& x) { return applyAvoidableRectangle(x); }, n)) { use(Strategy::AvoidableRectangle, n); progress = true; continue; }
        if (tryStep(Strategy::BUGPlus1, [&](int& x) { return applyBUGPlus1(x); }, n)) { use(Strategy::BUGPlus1, n); progress = true; continue; }
        if (tryStep(Strategy::RemotePairs, [&](int& x) { return applyRemotePairs(x); }, n)) { use(Strategy::RemotePairs, n); progress = true; continue; }
        if (tryStep(Strategy::WWing, [&](int& x) { return applyWWing(x); }, n)) { use(Strategy::WWing, n); progress = true; continue; }
        if (enableExperimentalGroupedXCycle &&
            tryStep(Strategy::GroupedXCycle, [&](int& x) { return applyGroupedXCycle(x); }, n)) { use(Strategy::GroupedXCycle, n); progress = true; continue; }
        if (tryStep(Strategy::XChain, [&](int& x) { return applyXChain(x); }, n)) { use(Strategy::XChain, n); progress = true; continue; }
        if (enableExperimentalXYChain &&
            tryStep(Strategy::XYChain, [&](int& x) { return applyXYChain(x); }, n)) { use(Strategy::XYChain, n); progress = true; continue; }
        if (tryStep(Strategy::GroupedAIC, [&](int& x) { return applyGroupedAIC(x); }, n)) { use(Strategy::GroupedAIC, n); progress = true; continue; }
        if (tryStep(Strategy::AIC, [&](int& x) { return applyAIC(x); }, n)) { use(Strategy::AIC, n); progress = true; continue; }
        if (tryStep(Strategy::ContinuousNiceLoop, [&](int& x) { return applyContinuousNiceLoop(x); }, n)) { use(Strategy::ContinuousNiceLoop, n); progress = true; continue; }
        if (tryStep(Strategy::SKLoop, [&](int& x) { return applySKLoop(x); }, n)) { use(Strategy::SKLoop, n); progress = true; continue; }
        if (tryStep(Strategy::ALSXZ, [&](int& x) { return applyALSXZ(x); }, n)) { use(Strategy::ALSXZ, n); progress = true; continue; }
        if (tryStep(Strategy::ALSXYWing, [&](int& x) { return applyALSXYWing(x); }, n)) { use(Strategy::ALSXYWing, n); progress = true; continue; }
        if (tryStep(Strategy::ALSChain, [&](int& x) { return applyALSChain(x); }, n)) { use(Strategy::ALSChain, n); progress = true; continue; }
        if (tryStep(Strategy::DeathBlossom, [&](int& x) { return applyDeathBlossom(x); }, n)) { use(Strategy::DeathBlossom, n); progress = true; continue; }
        if (tryStep(Strategy::SueDeCoq, [&](int& x) { return applySueDeCoq(x); }, n)) { use(Strategy::SueDeCoq, n); progress = true; continue; }
        if (tryStep(Strategy::MSLS, [&](int& x) { return applyMSLS(x); }, n)) { use(Strategy::MSLS, n); progress = true; continue; }
        if (tryStep(Strategy::Exocet, [&](int& x) { return applyExocet(x); }, n)) { use(Strategy::Exocet, n); progress = true; continue; }
        if (tryStep(Strategy::SeniorExocet, [&](int& x) { return applySeniorExocet(x); }, n)) { use(Strategy::SeniorExocet, n); progress = true; continue; }
        if (tryStep(Strategy::PatternOverlayMethod, [&](int& x) { return applyPatternOverlayMethod(x); }, n)) { use(Strategy::PatternOverlayMethod, n); progress = true; continue; }
        if (tryStep(Strategy::ForcingChains, [&](int& x) { return applyForcingChains(x); }, n)) { use(Strategy::ForcingChains, n); progress = true; continue; }
    }
}
...
AnalysisReport SudokuAnalyzer::run(int logical_max_rank, bool enable_backtracking) {
    ...
    strategy_bug_rejections_.fill(0);
    strategy_bug_logs_.clear();
    ...
    r.strategy_bug_rejections = strategy_bug_rejections_;
    r.debug_logic_logs = debug_logic_logs_;
    r.strategy_bug_logs = strategy_bug_logs_;
...
BacktrackingCounter::BacktrackingCounter(int br, int bc, int n, std::vector<int> grid)
    : BR_(br), BC_(bc), N_(n), NN_(n * n), all_((1ULL << n) - 1ULL), grid_(std::move(grid)) {}
...
static SudokuBoard parseSudokuLine(const std::string& line) {
    ...
    b.source_solution.reserve(b.total_cells);
    for (int i = 0; i < b.total_cells; ++i) {
        ...
        if (tok.empty() || tok == "0" || tok == "x" || tok == "X") {
            ...
            if (!parseIntStrict(tok.substr(1), v) || v < 1 || v > b.side_size) {
                b.error = "Niepoprawna wskazowka"; b.cells.clear(); b.source_solution.clear(); return b;
            }
            ...
            if (!parseIntStrict(tok, v) || v < 1 || v > b.side_size) {
                b.error = "Niepoprawna wartosc komorki"; b.cells.clear(); b.source_solution.clear(); return b;
            }
            c.value = 0; c.revealed = false;
            sourceValue = v;
        }
        b.cells.push_back(c);
        b.source_solution.push_back(sourceValue);
    }
    b.valid = true;
    return b;
}
...
static std::string formatSudokuGrid(const std::vector<int>& values, int sideSize, int blockRows, int blockCols) {
    if (sideSize <= 0 || blockRows <= 0 || blockCols <= 0) {
        return "(niepoprawny rozmiar planszy)";
    }
    ...
    for (int r = 0; r < sideSize; ++r) {
        ...
        for (int c = 0; c < sideSize; ++c) {
            ...
            const int idx = r * sideSize + c;
            const int v = (idx >= 0 && idx < static_cast<int>(values.size())) ? values[idx] : 0;
            row << v;
        }

        const std::string rowLine = row.str();
        out << rowLine;
        if (r + 1 < sideSize) {
            out << "\n";
        }
        if ((r + 1) < sideSize && ((r + 1) % blockRows == 0)) {
            out << std::string(rowLine.size(), '-') << "\n";
        }
    }
    return out.str();
}
...
static void writePerPuzzleSolveOutput(const fs::path& outDir, const PuzzleResult& result) {
    ...
    fs::path relFolder = result.relative_folder;
    if (relFolder.empty()) {
        relFolder = ".";
    }

    std::error_code ec;
    const fs::path targetDir = outDir / relFolder;
    fs::create_directories(targetDir, ec);
    if (ec) {
        std::cerr << "Nie mozna utworzyc katalogu wynikow Sudoku: " << targetDir.string() << "\n";
        return;
    }

    std::string baseName = fs::path(result.source_file).stem().string();
    if (baseName.empty()) {
        baseName = "sudoku";
    }
    const std::string outName = sanitizeFileName(baseName + "_linia_" + std::to_string(result.line_no) + "_wynik.txt");
    const fs::path outPath = targetDir / outName;

    std::ofstream out(outPath);
    if (!out.is_open()) {
        std::cerr << "Nie mozna zapisac pliku Sudoku: " << outPath.string() << "\n";
        return;
    }

    std::vector<int> puzzle(result.board.total_cells, 0);
    for (int i = 0; i < result.board.total_cells && i < static_cast<int>(result.board.cells.size()); ++i) {
        if (result.board.cells[i].revealed) {
            puzzle[i] = result.board.cells[i].value;
        }
    }

    out << "Sudoku do rozwiazania:\n";
    out << formatSudokuGrid(puzzle, result.board.side_size, result.board.block_rows, result.board.block_cols) << "\n\n";

    out << "Plansza wczytana do pamieci solvera (przed rozwiazywaniem):\n";
    const bool hasSolverInput = static_cast<int>(result.report.solver_input_grid.size()) == result.board.total_cells;
    if (hasSolverInput) {
        out << formatSudokuGrid(result.report.solver_input_grid, result.board.side_size,
                                result.board.block_rows, result.board.block_cols) << "\n\n";
    } else {
        out << formatSudokuGrid(puzzle, result.board.side_size, result.board.block_rows, result.board.block_cols) << "\n\n";
    }

    out << "Sudoku z pliku (rozwiazanie):\n";
    const bool hasSourceSolution = static_cast<int>(result.board.source_solution.size()) == result.board.total_cells;
    if (hasSourceSolution) {
        out << formatSudokuGrid(result.board.source_solution, result.board.side_size,
                                result.board.block_rows, result.board.block_cols) << "\n\n";
    } else {
        out << "(brak pelnego rozwiazania w danych wejsciowych)\n\n";
    }

    out << "Proba solvera:\n";
    out << "- Rozwiazane logicznie: " << (result.report.solved_logically ? "TAK" : "NIE") << "\n";
    out << "- Wymaga zgadywania/backtrackingu: " << (result.report.requires_guessing ? "TAK" : "NIE") << "\n";
    out << "- Rozwiazane backtrackingiem: " << (result.report.solved_with_backtracking ? "TAK" : "NIE") << "\n";
    out << "- Liczba rozwiazan (limit 2): " << result.report.solution_count << "\n";
    out << "- Jedno rozwiazanie: " << (result.report.unique_solution ? "TAK" : "NIE") << "\n\n";

    out << "Wykryte bledy strategii (lv1-lv9):\n";
    long long totalRejected = 0;
    for (int lv = 1; lv <= 9; ++lv) {
        const long long cnt = result.report.strategy_bug_rejections[lv];
        totalRejected += cnt;
        out << "- lv" << lv << ": " << cnt << "\n";
    }
    if (totalRejected == 0) {
        out << "- brak ruchow sprzecznych z rozwiazaniem z pliku\n";
    }
    out << "\n";

    if (!result.report.strategy_bug_logs.empty()) {
        out << "Szczegoly wykrytych bledow strategii:\n";
        for (const std::string& bugLine : result.report.strategy_bug_logs) {
            out << bugLine << "\n";
        }
        out << "\n";
    }

    out << "Rozwiazanie znalezione przez solver:\n";
    const bool hasSolverSolution = static_cast<int>(result.report.solved_grid.size()) == result.board.total_cells;
    if (hasSolverSolution) {
        out << formatSudokuGrid(result.report.solved_grid, result.board.side_size,
                                result.board.block_rows, result.board.block_cols) << "\n";
        if (hasSourceSolution) {
            out << "\nZgodnosc rozwiazania solvera z rozwiazaniem z pliku: "
                << ((result.report.solved_grid == result.board.source_solution) ? "TAK" : "NIE") << "\n";
        }
    } else {
        out << "(solver nie znalazl rozwiazania)\n";
    }

    out << "\n\nKroki solvera (logika):\n";
    if (result.report.debug_logic_logs.empty()) {
        out << "(brak logow krokow)\n";
    } else {
        for (const std::string& logLine : result.report.debug_logic_logs) {
            out << logLine << "\n";
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    try {
        const bool cliMode = (argc >= 2 && argv != nullptr && argv[1] != nullptr);
        const std::string input = cliMode ? std::string(argv[1]) : selectFolderModern();
        if (input.empty()) {
            if (cliMode) {
                std::cerr << "Nie podano folderu wejsciowego.\n";
            } else {
        ...
        const fs::path perPuzzleSolveDir = outDir / "rozwiazania_sudoku";
        ...
        fs::remove_all(perPuzzleSolveDir);
        fs::create_directories(folderReportsDir);
        fs::create_directories(perPuzzleSolveDir);
        ...
        for (const fs::path& currentPath : txtFiles) {
            ...
            while (std::getline(in, line)) {
                ...
                PuzzleTask task;
                task.folder_key = folderKey;
                task.relative_folder = relFolder;
                task.source_file = sourceFileName;
                task.source_path = currentPath.string();
                task.line_no = line_no;
                task.clean_line = clean;
                tasks.push_back(std::move(task));
            }
        }

        const long long totalEntries = static_cast<long long>(tasks.size());
        const ULONGLONG progressTotal = static_cast<ULONGLONG>(totalEntries > 0 ? totalEntries : 1);
        std::size_t workerCount = 0;
        if (!tasks.empty()) {
            const unsigned int hw = std::thread::hardware_concurrency();
            workerCount = static_cast<std::size_t>(hw > 0 ? hw : 4U);
            if (workerCount == 0) {
                workerCount = 1;
            }
            if (workerCount > tasks.size()) {
                workerCount = tasks.size();
            }
        }
        ...
        if (SUCCEEDED(CoCreateInstance(
            ...
            const std::wstring line3 = L"Tryb wielowatkowy, watki: " + std::to_wstring(workerCount);
            progress->SetLine(1, line1.c_str(), FALSE, nullptr);
            progress->SetLine(2, line2.c_str(), FALSE, nullptr);
            progress->SetLine(3, line3.c_str(), FALSE, nullptr);
        ...
        std::vector<PuzzleResult> results(tasks.size());
        ...
        for (std::size_t i = 0; i < workerCount; ++i) {
            workers.emplace_back([&]() {
                while (true) {
                    ...
                    const std::size_t idx = nextTask.fetch_add(1, std::memory_order_relaxed);
                    if (idx >= tasks.size()) {
                        break;
                    }
                    results[idx] = analyzePuzzleTask(tasks[idx]);
        ...
        std::size_t lastShown = static_cast<std::size_t>(-1);
        auto lastUiTick = std::chrono::steady_clock::now();

        if (!tasks.empty()) {
            while (true) {
                const std::size_t done = completed.load(std::memory_order_relaxed);
                const std::size_t claimed = nextTask.load(std::memory_order_relaxed);
                processedEntries = static_cast<long long>(done);

                if (progress != nullptr) {
                    const auto now = std::chrono::steady_clock::now();
                    const bool timeRefresh = (now - lastUiTick) >= std::chrono::milliseconds(250);
                    const bool countRefresh = (done != lastShown) &&
                        (done == 1 || done % 25 == 0 || done == tasks.size());
                    if (timeRefresh || countRefresh) {
                        const std::wstring p1 = L"Przeanalizowano Sudoku: " + std::to_wstring(done)
                            + L" / " + std::to_wstring(tasks.size());
                        const std::wstring p2 = L"Aktywne watki CPU: " + std::to_wstring(workerCount);
                        progress->SetLine(1, p1.c_str(), FALSE, nullptr);
                        progress->SetLine(2, p2.c_str(), FALSE, nullptr);
                        progress->SetProgress64(static_cast<ULONGLONG>(done), progressTotal);
                        lastShown = done;
                        lastUiTick = now;
                    }
                    if (!cancelled && progress->HasUserCancelled()) {
                        cancelled = true;
                        stopRequested.store(true, std::memory_order_relaxed);
                    }
                }

                const bool finished = cancelled ? (done >= claimed) : (done >= tasks.size());
                if (finished) {
                    break;
                }
        ...
        processedEntries = static_cast<long long>(completed.load(std::memory_order_relaxed));

        long long totalInvalid = 0;
        long long totalAnalyzed = 0;
        for (const PuzzleResult& result : results) {
            if (!result.processed) {
                continue;
            }
            FolderStats& folderStats = statsByFolder[result.folder_key];
            folderStats.relative_folder = result.relative_folder;
            if (!result.valid) {
                ++folderStats.invalid_lines;
                ++totalInvalid;
                appendInvalidPuzzleReport(folderStats, result.source_file, result.line_no, result.error);
                std::cerr << "Pominieto wpis (" << result.source_path << ":" << result.line_no
                          << "): " << result.error << "\n";
                continue;
            }
            updateFolderStats(folderStats, result.report);
            appendValidPuzzleReport(folderStats, result.source_file, result.line_no, result.board, result.report);
            writePerPuzzleSolveOutput(perPuzzleSolveDir, result);
            ++totalAnalyzed;
        }
        ...
        msg << "Przeskanowane pliki .txt: " << txtFilesScanned
            ...
            << "\nRozwiazania per Sudoku: " << perPuzzleSolveDir.string()
        ...
        if (argc >= 2) {
            std::cerr << "Blad krytyczny: " << ex.what() << "\n";
        } else {
